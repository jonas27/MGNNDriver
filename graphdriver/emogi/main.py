        ...
        **kwargs
        ...
        with tf.variable_scope(self.name + "_vars"):
            ...
            for i in range(len(self.support)):
                self.vars["weights_" + str(i)] = glorot_3d(dims, name="weights_" + str(i))
            if self.bias:
...
class EMOGI(Model):
    ...
    def _loss(self):
        ...
        # Cross entropy error
        self.loss += self.masked_softmax_cross_entropy_weight(
    ...
    def masked_softmax_cross_entropy_weight(self, scores, labels, mask):
        ...
        if scores.shape[1] > 1:  # softmax activation in last layer, no weights
            ...
            loss = tf.nn.weighted_cross_entropy_with_logits(
                targets=labels, logits=prediction, pos_weight=self.pos_loss_multiplier
            )
    ...
    def get_performance_metrics(self):
        with tf.variable_scope("evaluation"):
            ...
            _, auroc = tf.metrics.auc(
                labels=self.placeholders["labels"],
                predictions=pred,
                weights=self.placeholders["labels_mask"],
                curve="ROC",
            )
            _, aupr = tf.metrics.auc(
                labels=self.placeholders["labels"],
                predictions=pred,
                weights=self.placeholders["labels_mask"],
                curve="PR",
